...
class Renderer(nn.Module):
    ...
    def forward(self, pixels, camera_mat, world_mat, scale_mat, 
                      ...
                      it=0,light_src=None, novel_view=False, view_ori=None,**kwargs):
        if rendering_technique == 'unisurf':
            out_dict = self.unisurf(
                pixels, camera_mat, world_mat, 
                scale_mat, light_src=light_src, it=it, add_noise=add_noise, eval_=eval_,
                novel_view=novel_view, view_ori=view_ori,**kwargs,
            )
    ...
    def unisurf(self, pixels, camera_mat, world_mat, 
                scale_mat, light_src=None, add_noise=False, it=100000, eval_=False, 
                novel_view=False, view_ori=None, **kwargs):
        # Get configs
        ...
        if novel_view:
            light_s = F.normalize(camera_world[0],dim=-1).unsqueeze(1)
            diffuse_per = torch.tensor([0.7,0.7,0.7]).float().to(device)
            ambiant = torch.tensor([0.3,0.3,0.3]).float().to(device)
            surface = torch.ones_like(points).to(device)
            diffuse = torch.mm(normals_[:N], light_s).clamp_min(0).repeat(1, 3) * diffuse_per.unsqueeze(0)
            surface[surface_mask] = (ambiant.unsqueeze(0) + diffuse).clamp_max(1.0)
            out_dict = {
                'surface': surface.reshape(batch_size, -1, 3),
            }
            return out_dict
        ...
        for i in range(0, surface_points.shape[0], n_max_network_queries):
            rgb_i, logits_alpha_i, albedo_i, spec_i, sg_weights_i = self.model(
        ...
        rgb_fine = (rgb_fine * vis[...,None]).clamp(0,1)
                
        out_dict = {
            'mask_pred': network_object_mask,
            'normal_pred': normal.reshape(batch_size, -1, 3),
            'depth' : depth.reshape(batch_size,-1),
            'vis' : vis_map.reshape(batch_size, -1),
            'rgb_fine': rgb_fine.reshape(batch_size,-1,3),
            'albedo_fine': albedo_fine.reshape(batch_size, -1, 3),
            'specular_fine': spec_fine.reshape(batch_size, -1, 3),
        }
